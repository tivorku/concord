...
func InitLogicCore(l *Ledger, myLotID string, vol int, val int, privKey crypto.PrivKey, bearer, number string, node *Node) *LogicCore {
	/*licenseFile, err := os.ReadFile("license.bin")
	if err != nil {
		if strings.Contains(err.Error(), "no such file or directory") {
			fmt.Println("Файл 'license.bin' не найден.\nВаш PeerID:", node.Host.ID())
		}
		os.Exit(1)
	}
	if !VerifyLicense(licenseFile, node.Host.ID()) {
		fmt.Println("Лицензия не действительна.")
		os.Exit(1)
	}*/
	return &LogicCore{
		ledger:         l,
...
func (lc *LogicCore) SelectRandomProxy(h host.Host, ctx context.Context) (peer.ID, string) {
	...
	for _, pid := range h.Network().Peers() {
		if pid == h.ID() || pid.String() == relayID.String() {
			continue
		}
		isMDNMember := false
